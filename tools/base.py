...
from misc.losses import FocalLoss
...
class BaseRunner():
    ...
    def initialize(self, LR):
        ...
        if self.cfg.TRAINING.optimizer == 'sgd':
            ...
            self.optimizer = optim.Adam(filter(lambda p: p.requires_grad, self.model.parameters()), lr=LR, betas=(0.9, 0.999), weight_decay=1e-4)
            ...
            self.optimizer = optim.AdamW(self.model.parameters(), lr=LR, betas=(0.9, 0.999), weight_decay=1e-4)
    ...
    def saveModelWeight(self, epoch, acc, f1_score_avg):
        saveGroup = {
            'epoch': epoch,
        ...
        if self.logger.isBestAccAP(acc):
            ...
            print('==========>Save the best model, Accuracy is {}...'.format(saveGroup['accuracy']))
            torch.save(saveGroup, os.path.join(self.dir, 'model_best.pth'))

        if self.logger.isBestF1Score(f1_score_avg):
            saveGroup['f1_score_avg'] = self.logger.showBestF1Score()
            print('==========>Save the best model, Accuracy is {}, F1_score is {} ...'.format(saveGroup['accuracy'],saveGroup['f1_score_avg']))
            torch.save(saveGroup, os.path.join(self.dir, 'model_best.pth'))
    ...
    def loadModelWeight(self, mode):
        ...
        if os.path.isdir(self.dir) and os.path.exists(checkpoint):
            ...
            # net_dict_key = []
            # for key in net_dict:
            #     net_dict_key.append(key)
            # # 读取预训练模型权重
            # checkpoint = torch.load(checkpoint)
            # checkpoints = checkpoint['model_state_dict']
            # # 寻找网络公共层,更新至已修改好的model
            # stat_dict = {k: v for k, v in checkpoints.items() if k in net_dict_key}
            # net_dict.update(stat_dict)
            # #加载预训练模型
